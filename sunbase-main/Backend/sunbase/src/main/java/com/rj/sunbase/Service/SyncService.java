...
public class SyncService {
	...
	public String proxyAuthToken() {
		...
		String jsonPayload = String.format("{\"login_id\":\"%s\",\"password\":\"%s\"}", STATIC_LOGIN_ID,
				STATIC_PASSWORD);
	...
	public List<Customer> extractCustomerList() {
		...
		ResponseEntity<Customer[]> response = restTemplate.exchange(builder.toUriString(), HttpMethod.GET, request,
				Customer[].class);
	...
	public List<Customer> syncCustomerListFromApi() {
		...
		for (Customer customer : customerList) {
			...
			if (!exists) {
				...
				log.warn("Customer with ID " + customer.getUuid() + " already exists in the database.");
			}
		}

		if (allExist) {
			throw new RuntimeException("All customers already exist in the database.");
		}
